...
class Transaction {
public:
    Transaction() : who(""), when(nullptr), amount(0) {}
     ...
     * copy constructor
     */
    Transaction(const Transaction &trans) : who(trans.who), amount(trans.amount), when(copy_unique(trans.when)) {}
     ...
     * move constructor
     */
    Transaction(Transaction &&trans) : who(move(trans.who)), amount(move(trans.amount)), when(move(trans.when)) {}
    ...
    Transaction &operator=(Transaction &&trans) {
        who = move(trans.who);
        amount = move(trans.amount);
        when = move(trans.when);
    }
